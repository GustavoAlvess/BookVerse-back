...
export const criar = async (req, res) => {
  try {
    ...
    if (!objetivo) {
      return res
        .status(400)
        .json({ error: 'O campo "objetivo" é obrigatório!' });
    }
    if (!curso) {
    ...
    if (!fotoURL) {
      return res
        .status(400)
        .json({ error: 'O campo "fotoURL" é obrigatório!' });
    }
    ...
    return res
      .status(201)
      .json({ message: "Membro criado com sucesso!", data });
  } catch (error) {
...
export const atualizar = async (req, res) => {
  try {
    ...
    if (!membro) {
      return res
        .status(404)
        .json({ error: "Membro não encontrado para atualizar." });
    }
    ...
    return res
      .status(200)
      .json({
        message: `O membro "${data.nome}" foi atualizado com sucesso!`,
        data,
      });
  } catch (error) {
...
export const deletar = async (req, res) => {
  try {
    ...
    if (!membro) {
      return res
        .status(404)
        .json({ error: "Membro não encontrado para deletar." });
    }
