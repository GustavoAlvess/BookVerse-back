  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        error:
          "Todos os campos de livro_id, categoria, título e conteúdo (PT/EN) são obrigatórios!",
      });
    }
    ...
    if (
      existentes.some(
        (c) => c.titulo_pt.toLowerCase() === titulo_pt.toLowerCase(),
      )
    ) {
      return res
        .status(400)
        .json({
          error: `A curiosidade "${titulo_pt}" já existe para este livro!`,
        });
    }
    ...
    return res
      .status(201)
      .json({ message: "Curiosidade criada com sucesso!", data });
  } catch (error) {
    console.error("Erro ao criar curiosidade:", error);
    return res
      .status(500)
      .json({ error: "Erro interno ao salvar curiosidade." });
  }
};
...
export const atualizar = async (req, res) => {
  try {
    ...
    if (req.body.titulo_pt && req.body.titulo_pt !== curiosidade.titulo_pt) {
      const existentes = await CuriosidadeModel.buscarTodos({
        livro_id: curiosidade.livro_id,
      });
      if (existentes.some((c) => c.titulo_pt === req.body.titulo_pt)) {
        return res
          .status(400)
          .json({ error: "Já existe outra curiosidade com este título." });
      }
...
export const deletar = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Curiosidade removida com sucesso!" });
  } catch (error) {
