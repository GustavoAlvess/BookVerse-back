...
export const atualizar = async (req, res) => {
  try {
    ...
    if (!usuario) {
      return res
        .status(404)
        .json({ error: "Usuario não encontrado para atualizar." });
    }
    ...
    if (req.body.senha_hash !== undefined)
      usuario.senha_hash = req.body.senha_hash;
    ...
    return res
      .status(200)
      .json({
        message: `O usuario "${data.nome}" foi atualizado com sucesso!`,
        data,
      });
  } catch (error) {
...
export const deletar = async (req, res) => {
  try {
    ...
    if (!usuario) {
      return res
        .status(404)
        .json({ error: "Usuario não encontrado para deletar." });
    }
