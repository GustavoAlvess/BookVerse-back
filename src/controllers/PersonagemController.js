...
export const criar = async (req, res) => {
  try {
    ...
    const { livro_id, nome, descricao_pt, descricao_en, papel, imagem_url } =
      req.body;

    if (!livro_id) {
      return res.status(400).json({
        error: 'O campo "livro_id" é obrigatório para vincular o personagem!',
      });
    }
    if (!nome) {
    ...
    if (!descricao_pt || !descricao_en) {
      return res.status(400).json({
        error:
          "As descrições em PT e EN são obrigatórias (Internacionalização)!",
      });
    }
    ...
    return res
      .status(500)
      .json({ error: "Erro interno ao salvar o personagem." });
  }
};
...
export const atualizar = async (req, res) => {
  try {
    ...
    if (req.body.imagem_url !== undefined)
      personagem.imagem_url = req.body.imagem_url;
    if (req.body.livro_id !== undefined)
      personagem.livro_id = parseInt(req.body.livro_id);

    const data = await personagem.atualizar();

    return res.status(200).json({
      message: `O personagem "${data.nome}" foi atualizado com sucesso!`,
      data,
    });
  } catch (error) {
