  ...
  try {
    ...
    if (!movimento_pt && !movimento_en) {
      return res
        .status(400)
        .json({
          error: 'O campo "movimento_pt & movimento_en" é obrigatório!',
        });
    }
    if (!descricao_pt && !descricao_en) {
      return res
        .status(400)
        .json({
          error: 'O campo "descricao_pt & descricao_en" é obrigatório!',
        });
    }
...
export const atualizar = async (req, res) => {
  try {
    ...
    if (req.body.movimento_pt !== undefined)
      livro.movimento_pt = req.body.movimento_pt;

    if (req.body.movimento_en !== undefined)
      livro.movimento_en = req.body.movimento_en;

    if (req.body.descricao_pt !== undefined)
      livro.descricao_pt = req.body.descricao_pt;

    if (req.body.descricao_en !== undefined)
      livro.descricao_en = req.body.descricao_en;
